...
class DiffieHellman:
    ...
    # getter и setter для публичного ключа
    ...
    def calculatePublicKey(self):
        ...
        # a^x mod r
        self.publicKey = pow(self.a, self.x, self.r)
        print("[ Public key for {}] {} ".format(self.name, hex(self.publicKey)))

    # Вычисление секретного ключа пользователя
    def calculatePrivateKey(self):
        print("[ Calculating private key for {} ]".format(self.name))
        
        # a^xy mod r
        self.privateKey = pow(self.getPublicKey(), self.x, self.r)
        print("[ Private key for {}] {} ".format(self.name, hex(self.privateKey)))
...
def asn_encoder(p_key, a, r):
    ...
    # Последовательность -- первый ключ DH
    encoder.enter(asn1.Numbers.Sequence)
        
    # Идентификатор DH
    encoder.write(b'0x0021', asn1.Numbers.OctetString)
    # Необзяталеьный идентификатор ключа
    encoder.write(b'0xdh', asn1.Numbers.UTF8String)
    
    # DH - значение открытого ключа
    encoder.enter(asn1.Numbers.Sequence)
    encoder.leave()
        
    # Последовательность, содержащая параметры криптосистемы - r, a
    encoder.enter(asn1.Numbers.Sequence)  
    ...
    # DH - значение открытого ключа
    encoder.enter(asn1.Numbers.Sequence)
    encoder.write(p_key, asn1.Numbers.Integer)
    encoder.leave()
   
    # Выход из множества ключа DH
    encoder.leave()
